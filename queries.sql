...
FROM
	...
	JOIN inventory t4 on t3.film_id = t4.film_id
	JOIN rental t5 on t4.inventory_id = t5.inventory_id
	JOIN payment t6 on t5.rental_id = t6.rental_id
GROUP BY
	T1.NAME
ORDER BY
	TOTAL_RENTAL_COST DESC
LIMIT 1;
...
WITH
	GROUPED_CTE AS (
		...
		FROM
			...
			WHERE T4.return_date IS NOT NULL
		GROUP BY
			T1.CITY,
			T8.NAME
		ORDER BY
			TOTAL_RENT_TIME DESC
	),
	RANKED_CTE AS (
		SELECT
			GROUP_CITY,
