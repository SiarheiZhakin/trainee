...
# df_film = load_table('film_category', 't2')
# df_result_1 = df_category.join(df_film, how='left', on='category_id)\
...
#4
# df_result_4 = df_film.join(df_inventory, on='film_id', how='left_anti')\
#                      .select('title')\
#                      .distinct()\
...
#             .join(df_category, on='category_id', how='inner').cache()
# df.count()
# num_partitions_a = df.rdd.getNumPartitions()
# print(f'{num_partitions_a} partitions before group')
...
#         when(lower(col("city")).startswith("a"), "start_a")
#          .when(col("city").contains("-"), "have -")
#     )
# num_partitions_b = grouped_df.rdd.getNumPartitions()
# print(f'{num_partitions_b} partitions after group')
#
